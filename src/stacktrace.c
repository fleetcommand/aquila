...
static void GCC_DumpStack (void)
{
  ...
  for (i = 0; p; i++) {
	...
	break;
      case 4:
	p = __builtin_frame_address (4);
	if (p)
	  p = __builtin_return_address (4);
	break;
      case 5:
	p = __builtin_frame_address (5);
	if (p)
	  p = __builtin_return_address (5);
	break;
      case 6:
	p = __builtin_frame_address (6);
	if (p)
	  p = __builtin_return_address (6);
	break;
      case 7:
	p = __builtin_frame_address (7);
	if (p)
	  p = __builtin_return_address (7);
	break;
      case 8:
	p = __builtin_frame_address (8);
	if (p)
	  p = __builtin_return_address (8);
	break;
      case 9:
	p = __builtin_frame_address (9);
	if (p)
	  p = __builtin_return_address (9);
	break;
      case 10:
	p = __builtin_frame_address (10);
	if (p)
	  p = __builtin_return_address (10);
	break;
      case 11:
	p = __builtin_frame_address (11);
	if (p)
	  p = __builtin_return_address (11);
	break;
      case 12:
	p = __builtin_frame_address (12);
	if (p)
	  p = __builtin_return_address (12);
	break;
      case 13:
	p = __builtin_frame_address (13);
	if (p)
	  p = __builtin_return_address (13);
	break;
      case 14:
	p = __builtin_frame_address (14);
	if (p)
	  p = __builtin_return_address (14);
	break;
      case 15:
	p = __builtin_frame_address (15);
	if (p)
	  p = __builtin_return_address (15);
	break;
      case 16:
	p = __builtin_frame_address (16);
	if (p)
	  p = __builtin_return_address (16);
	break;
      case 17:
	p = __builtin_frame_address (17);
	if (p)
	  p = __builtin_return_address (17);
	break;
      case 18:
	p = __builtin_frame_address (18);
	if (p)
	  p = __builtin_return_address (18);
	break;
      case 19:
	p = __builtin_frame_address (19);
	if (p)
  ...
  if (SYS_ERROR != fd) {
	...
	if ((type == 't') || type == 'T') {
	  ...
	  for (i = 0; syms[i].realAddress != 0; i++) {
	    if ((addr <= syms[i].realAddress) && (addr > syms[i].closestAddress)) {
    ...
    for (i = 0; syms[i].realAddress != 0; i++) {
	  ...
	  (syms[i].realAddress <= lowestAddress) || (syms[i].realAddress >= highestAddress)) {
	sprintf (buffer, "[%d] 0x%08lx ???\n", i, syms[i].realAddress);
      } else {
	sprintf (buffer, "[%d] 0x%08lx <%s + 0x%lx> %c\n",
		 i,
		 syms[i].realAddress,
		 syms[i].name, syms[i].realAddress - syms[i].closestAddress, syms[i].type);
      }
...
static int DumpStack (char *format, ...)
{
  ...
  if (SYS_ERROR != fd) {
	...
	buffer = buf;
	if (!gotSomething) {
	  write (global_output, "Output from ", strlen ("Output from "));
	  strtok (cmd, " ");
...
# endif
  ...
  write (global_output, "No debugger found\n", strlen ("No debugger found\n"));
